...
public class Sink extends RoomElement implements Prototype {
    ...
    public Sink(DraftNode parent, double xRatio, double yRatio, int rotateRatio) {
        super("Sink" + counter, parent, xRatio, yRatio, rotateRatio);
        this.name = "Sink" + counter;
        ++counter;
    }
    ...
    @Override
    public Sink clone() {
        Sink cloned = new Sink(getParent(), getXRatio(), getYRatio(), getRotateRatio());
        cloned.setY(getY());
        cloned.setX(getX() + 20);
