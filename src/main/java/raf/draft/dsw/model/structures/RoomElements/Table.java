...
public class Table extends RoomElement implements Prototype {
    ...
    public Table(DraftNode parent, double xRatio, double yRatio, int rotateRatio) {
        super("Table" + counter, parent, xRatio, yRatio, rotateRatio);
    ...
    @Override
    public Table clone() {
        Table cloned = new Table(getParent(), getXRatio(), getYRatio(), getRotateRatio());
        cloned.setY(getY());
        cloned.setX(getX() + 20);
