...
public class Boiler extends RoomElement{
    ...
    public Boiler(DraftNode parent, double xRatio, double yRatio, int rotateRatio) {
        super("Boiler" + counter, parent, xRatio, yRatio, rotateRatio);
        this.name = "Boiler" + counter;
        ++counter;
    }
    ...
    public void setRatio(double widthRatio, double heightRatio) {
        ...
        setHeightRatio(widthRatio);
    }

    @Override
    public Boiler clone() {
        Boiler cloned = new Boiler(getParent(), getXRatio(), getYRatio(), getRotateRatio());
        cloned.setY(getY());
        cloned.setX(getX() + 20);
