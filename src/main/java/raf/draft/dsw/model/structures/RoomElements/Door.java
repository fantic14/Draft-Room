...
public class Door extends RoomElement implements Prototype {
    ...
    public Door(DraftNode parent, double xRatio, double yRatio, int rotateRatio) {
        super("Door" + counter, parent, xRatio, yRatio, rotateRatio);
        this.name = "Door" + counter;
        ++counter;
    }
    ...
    @Override
    public Door clone() {
        Door cloned = new Door(getParent(), getXRatio(), getYRatio(), getRotateRatio());
        cloned.setY(getY());
        cloned.setX(getX() + 20);
